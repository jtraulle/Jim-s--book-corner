...
class HTMLInput{
	...
	function __toString(){
		...
		switch($this->type){
			case TEXT : return "<div class='clearfix {$this->class}'><label for='{$this->id}'>{$this->label}</label><div class='input'><input class='{$this->class}' type='text' value='{$this->value}' id='{$this->id}' name='{$this->id}' /><span class='help-inline'>{$this->message}</span> </div></div>" ; break;
