...
<div class="pagination center">
	<ul>
		{if $totalPages > 10 }
			...
			{if $pageCourante != 1 }
				<li class="prev"><a href="?{$chemin}&page={$pageCourante-1}">&larr; Précédent</a></li>
			{else}
			...
			{if $nbPagesAvant < 8}
				{section name=pages start=1 loop=$pageCourante}
			  			<li><a href="?{$chemin}&page={$smarty.section.pages.index}">{$smarty.section.pages.index}</a></li>
				{/section}
				<li class="active"><a href="?{$chemin}&page={$pageCourante}">{$pageCourante}</a></li>
			...
			{if $pageCourante < $totalPages }
				<li class="prev"><a href="?{$chemin}&page={$pageCourante+1}">Suivant &rarr;</a></li>
			{else}
			...
			{/if}
	  		...
	  		{section name=pages loop=$totalPages}
		  		{if $pageCourante == $smarty.section.pages.iteration}
		  			<li class="active"><a href="?{$chemin}&page={$smarty.section.pages.iteration}">{$smarty.section.pages.iteration}</a></li>
		  		{else}
		  			<li><a href="?{$chemin}&page={$smarty.section.pages.iteration}">{$smarty.section.pages.iteration}</a></li>
		  		{/if}
			{/section}
