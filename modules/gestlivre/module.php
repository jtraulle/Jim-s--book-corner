...
class gestlivre extends Module {
    ...
    public function action_index()
    {
        ...
        if(isset($_GET['lettre']) && ctype_alpha($_GET['lettre']) && strlen($_GET['lettre']) == 1){
            $lettre = $_GET['lettre'];
            $this->tpl->assign("lettre", $lettre);
        }else{
            $this->tpl->assign("lettre", "noletter");
        }
            
        
        if(isset($lettre)){
            $nbEnregistrementsParPage = 10;
            $totalPages = Outils::nbPagesTotales("livre LEFT JOIN auteur ON livre.numAuteur = auteur.numAuteur WHERE nomAuteur LIKE '".$lettre."%'", $nbEnregistrementsParPage);
            ...
            $this->tpl->assign("listeLivres", Livre::listeAlphabet($lettre, $pageCourante, 10));
        }else{
            $nbEnregistrementsParPage = 10;
